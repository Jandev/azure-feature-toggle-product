...
import type { FeatureToggle, EnvironmentType, UserRole } from '@/types';
...
export function Dashboard({
  ...
  // Filter toggles based on search and filter
  const filteredToggles = toggles.filter((toggle) => {
    // Apply search filter
    const matchesSearch =
      searchQuery === '' || toggle.name.toLowerCase().includes(searchQuery.toLowerCase());

    // Apply state filter
    const matchesFilter =
      filter === 'all' ||
      (filter === 'enabled' && toggle.enabled) ||
      (filter === 'disabled' && !toggle.enabled);
  ...
  // Empty state
  if (!isLoading && toggles.length === 0 && !error) {
    return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {/* Production Warning */}
        {currentResource.environmentType === 'production' && userRole === 'admin' && (
        ...
        <div className="space-y-3">
          {filteredToggles.map((toggle) => (
            <ToggleRow
              ...
              toggle={toggle}
              userRole={userRole}
              isUpdating={updatingToggleId === toggle.id}
              onToggle={onToggle}
            />
          ))}
        </div>
